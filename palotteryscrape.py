...
def exportPAScratcherRecs():

    url = 'https://www.palottery.state.pa.us/Scratch-Offs/Prizes-Remaining.aspx'
    ...
    tixurls = soup.find('table', id='remaining-prizes')
    tixlist = pd.read_html(io.StringIO(str(tixurls)))[0]
    tixlist.rename(columns={'Game #':'gameNumber','Game Name':'gameName'}, inplace=True)
    print(tixlist.columns)
    tixlist['gameNumber'] = tixlist['gameNumber'].str.replace(' NEW ','')
    print(tixlist)

    print(tixlist.loc[:,['gameName','gameNumber']])
    tixurls = tixurls.find_all('td')
    ...
    for t in tixurls: 
        ...
        if t.find('a') == None:
            ...
            gameURL = 'https://www.palottery.state.pa.us/'+str(t.find('a').get('href'))
            print(gameURL)
            r = requests.get(gameURL)
            response = r.text
            soup = BeautifulSoup(response, 'html.parser').find('div', class_='prizes-remaining-table')
            print(soup)
            tixdata = pd.read_html(io.StringIO(str(soup.find('table', class_='table-global'))))[0]
            overallodds = soup.find('p', class_='table-disclaimer').string.replace('Overall chances of winning a prize:','').strip().replace('1:','')
            print(tixdata)
            print(overallodds)
            print(tixdata)
    ...
    scratchersall.to_csv("./PAscratcherslist.csv", encoding='utf-8')
    ...
    scratchertables.to_csv("./PAscratchertables.csv", encoding='utf-8')
    ...
    ratingstable['Stats Page'] = "/pennsylvania-statistics-for-each-scratch-off-game"
    ratingstable.to_csv("./PAratingstable.csv", encoding='utf-8')
    # write to Google Sheets
    # select a work sheet from its name
    PAratingssheet = gs.worksheet('KYRatingsTable')
    PAratingssheet.clear()
    ...
    print(ratingstable)
    set_with_dataframe(worksheet=PAratingssheet, dataframe=ratingstable, include_index=False,
    include_column_header=True, resize=True)
    return ratingstable, scratchertables

exportPAScratcherRecs()