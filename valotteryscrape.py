...
def exportScratcherRecs():
    ...
    for gameid in gamesgrouped['gameNumber']:
        ...
        print(gamerow.loc[:,'Max Tickets to Buy'])
...
allSimTables = pd.DataFrame(columns=['Game Number', 'Game Name', 'Cost', 'Longest Losing Streak', 'Optimal Bankroll', 'Risk Coefficient', 'Prize Types', 'Standard Deviations', 'Cluster Size',  
                                     'Ticket Number','Ticket Outcome','Cluster Number','Cluster Outcome','Total Tally'])
allSimOutcomes = pd.DataFrame(columns=['Game Number', 'Game Name', 'Cost', 'Longest Losing Streak', 'Optimal Bankroll', 'Risk Coefficient', 'Prize Types', 'Standard Deviations', 'Cluster Size', 'Any Prize Probability',
...
def clusterloop(ratingstable, scratchertables, prizetype, stddevs, riskCoeff):
    ...
    for gameid in ratingstable.loc[:,'gameNumber']:
        ...
        prizes = scratchertables.loc[(scratchertables['gameNumber']==gameid) & (scratchertables['prizeamount']!="Total"),'prizeamount'].astype('float')
        ...
        totalprizes['prizeamount'] = totalprizes['prizeamount'].astype('float')
        print(totalprizes)
        ...
        if (prizetype == "profit"):
            ...
            stDevpct = float(totalprizes.loc[(totalprizes['prizeamount']>price), 'Winning Tickets Unclaimed'].std().mean()/totaltixremain)
            stDev = float(totalprizes.loc[(totalprizes['prizeamount']>price), 'Winning Tickets Unclaimed'].std().mean())
            totalprizesremain = totalprizes.loc[(totalprizes['prizeamount']>price),'Winning Tickets Unclaimed'].sum()
            ...
            stDev = float(totalprizes.loc[totalprizes['prizeamount']>0, 'Winning Tickets Unclaimed'].std().mean())
            totalprizesremain = totalprizes.loc[totalprizes['prizeamount']>0,'Winning Tickets Unclaimed'].sum()
               
        print(totalprizes.dtypes)
        print(totalprizes.columns)
        print(oddsprizes)
        print(stddevs)
        print(totalprizesremain)
        print(stDev)
        print(stDevpct)
        print(gameprob)
        #probability plus std dev percent for max probability
        print(totaltixremain/(totaltixremain*(gameprob-(stDevpct*stddevs))))
        print(totaltixremain/(totalprizesremain-(stDev*stddevs)))
        ...
        if stddevs >= 0:
            clustersize = int(np.round(totaltixremain/(totalprizesremain-(stDev*abs(stddevs))),0))
        elif stddevs < 0:
            clustersize = int(np.round(totaltixremain/(totalprizesremain+(stDev*abs(stddevs))),0))
        print(clustersize) 
        ...
        while (clusterCount <= clusterSample):
            ...
            while tic <= len(clustergroup):
                ...
                allSimTables.loc[len(allSimTables)] = cluster
        ...
        allSimOutcomes.loc[len(allSimOutcomes)] = result
    ...
    allSimTables.to_csv("/Users/michaeljames/Documents/scratchersdatapull/simTable_Neg3StDevs.csv", encoding='utf-8')
    allSimOutcomes.to_csv("/Users/michaeljames/Documents/scratchersdatapull/simOutcomes_Neg3StDevs.csv", encoding='utf-8')
...
'''
ratingstable = pd.read_csv ('/Users/michaeljames/Documents/ScratcherDataTests/ratingstable_03082022.csv')
scratchertables = pd.read_csv ('/Users/michaeljames/Documents/ScratcherDataTests/scratchertables_03082022.csv')
